...
function comparePacketByIndex(i1, v1, i2, v2) {
...
END {
  ...
  # gawk doesn't seem able to sort array of arrays using `asort`. This:
  #
  # ```
  # asort(packets, sorted, "compare")
  # ```
  #
  # Dies with:
  #
  # > gawk: ./part2-bis.awk:145: (FILENAME=test FNR=23) fatal error: internal error
  # > Abort trap: 6

  # However, this works. Some sort of Schwartzian transform.
  asorti(packets, sorted, "comparePacketByIndex")

  product = 1

  for (i = 1; i <= packet_i; i++) {
    if ((sorted[i] == packet_i || sorted[i] == packet_i - 1)) {
      product *= i
    }
    # printf("%02s → %02s → %s\n", i, sorted[i], showArray(packets[sorted[i]]))
  }

  printf("Decoder key: %s\n", product)
}
